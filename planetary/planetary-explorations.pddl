...
(:durative-action move 
    ...
    (over all(not (is_analizing? ?rover)))(at start(is_on ?x)) (at start(> (battery-level ?rover) (* (speed ?rover) ?duration))))
    :effect (and(at end(decrease (battery-level ?rover) (* (speed ?rover) ?duration))) (at end(is_on ?y))(at end(not(is_moving? ?rover)))(at start (not (is_on ?x))) (at start (is_moving? ?rover)))
